...
**Claude Code 및 Gemini CLI 사용자의 경우**, 이 프로젝트는 이미 `.claude/` 폴더에 메모리 관리, 코드 가이드라인, PRD 생성, 작업 분해, 작업 업데이트 관련 규칙이 설정되어 있어 별도 설정 없이 바로 프로젝트 관리 기능을 사용할 수 있습니다. Claude Code와의 대화에서 "PRD 생성해줘", "작업 목록 만들어줘", "태스크 상태 업데이트해줘" 등의 명령을 통해 AI 어시스턴트와 체계적인 프로젝트 관리를 할 수 있습니다.

확인해보시려면 Claude Code나 Gemini CLI에게 다음과 같이 질문해보시면 확인 가능합니다.

```
"PRD 작성과 관련해서 너에게 주어진 규칙을 나에게 알려줘.
"너가 기억할 때 지켜야 하는 규칙이 있다면 나에게 알려줘.
```

![Claude Code Memory 예시](./images/claude-code-memory.png)
